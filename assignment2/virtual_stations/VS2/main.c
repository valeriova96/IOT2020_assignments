...
void getMessage(char* message){
    int temp = getRandom(-50,50);
    int hum = getRandom(0,100);
    int win_dir = getRandom(0,360);
    int win_int = getRandom(0,100);
    int rain = getRandom(0,50);

    sprintf(message, "{\"Temperature\":\"%d °C\",\"Humidity\":\"%d %%\",\"Wind direction\":\"%d degrees\",\"Wind intensity\":\"%d m/s\",\"Rain height\":\"%d mm/h\"}",temp,hum,win_dir,win_int,rain);

}


//Publishing random values
static int cmd_weather_pub(int argc, char **argv)
{
    emcute_topic_t t;
    unsigned flags = EMCUTE_QOS_0;
...
static const shell_command_t shell_commands[] = {
    ...
    { "weather", "publish a set of weather values", cmd_weather_pub },
